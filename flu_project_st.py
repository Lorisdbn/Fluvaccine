...
# Custom CSS to enforce the font throughout the app
st.markdown(
    """
    <style>
...
if page == pages[0]:
    ...
    st.text(" ")
    st.write("Can you predict whether individuals received the H1N1 and seasonal flu vaccines using information about their backgrounds, opinions, and health behaviors?")
    st.text(" ")
    st.write("In this challenge, we explore vaccination, a crucial public health strategy in the fight against infectious diseases. Vaccines not only protect individuals by providing immunization but also contribute to herd immunity, which helps curb the spread of diseases within a community.")
    # Display the image
    script_dir = os.path.dirname(os.path.abspath(__file__))
...
3. **Data processing for machine learning:** After gaining insights from exploration and visualization, we proceeded to preprocess the data for machine learning. This step included handling missing values, encoding categorical variables, scaling features, and selecting relevant features for model training.
...
""")
    ...
    with st.expander("Training set dataframe"):
        ...
        st.markdown("""The two last columns <u>h1n1_vaccine</u> and <u>seasonal_vaccine</u> are the target variable. We will train our model on this training data in order to be able to predict the vaccination rate on another unknown panel based on the 35 features we have""",unsafe_allow_html=True)
    ...
    **The features in this dataset**  
    You are provided with a dataset containing 36 columns. The first column, `respondent_id`, is a unique and random identifier. The remaining 35 features are described below:
    ...
    with st.expander("Training set dataframe types:"):
        ...
        st.text("")
        st.markdown("""Based on this information we can observe some categorical variables that will **need to be converted in order to train our future machine learning models.**""")
    ...
    with st.expander("Percentage of missing values per column:"):
        ...
        st.markdown("""Overall, the dataset seems to be of good quality, with most columns being nearly complete. This should facilitate robust statistical analysis and model building, as missing data handling can be kept to a minimum or dealt with straightforward methods like mean/median imputation or using models to predict missing values.""")
    ...
    if num_duplicates > 0:
        ...
        st.write("Nota bene : There is no duplicate rows found in the DataFrame.")
...
# Plot the respondents demographic pyramid  
    ...
    fig.add_trace(go.Bar(
        ...
        x=-age_sex_distribution['Male'],  # Negative to align left
        name='Male',
        orientation='h',
...
# Add bars for females (Sex 'Female')
    fig.add_trace(go.Bar(
        y=age_sex_distribution.index,
        x=age_sex_distribution['Female'],  # Positive to align right
        name='Female',
        orientation='h',
...
# Plot the respondents ethnical pyramid   
    ...
    fig.update_layout(
        ...
        plot_bgcolor='rgba(0, 0, 0, 0)', paper_bgcolor='rgba(0, 0, 0, 0)')
    # Display the chart in Streamlit
    with st.expander("Respondents demographic pyramid based on ethnic group"):
        st.plotly_chart(fig)
...
# Plot the concern level by age group
    ...
    for concern_level in concern_percentages.columns:
        fig.add_trace(go.Bar(
            ...
            text=concern_percentages[concern_level].round(1).astype(str) + '%',  # Add percentage labels
            textposition='auto',
...
# Plot the vaccination rates based on opinion
    ...
    st.text(" ")
    st.markdown("""Based on the previous insights, the main steps will be handling missing values and encoding numerical data.
                We will then do the same work on the test dataset to be sure the 2 dataframes are similarly processed""")
    st.text(" ")
